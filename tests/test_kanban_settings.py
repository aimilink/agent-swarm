...
def _default_settings(*, auto_dispatch_enabled: bool = False) -> dict:
    return {
        "auto_dispatch_enabled": auto_dispatch_enabled,
        "auto_dispatch_interval_ms": 2000,
        "multi_review_enabled": True,
        "block_human_input_enabled": True,
        "idempotent_protection_enabled": True,
    }
...
def test_settings_service_persists_collaboration_flags(monkeypatch, tmp_path):
    session_local = _session_local(tmp_path)
    monkeypatch.setattr("app.db.repositories.SessionLocal", session_local)
    service = SettingsService()

    assert service.get_kanban_multi_review_enabled() is True
    service.set_kanban_multi_review_enabled(False)
    service.set_kanban_block_human_input_enabled(False)
    service.set_kanban_idempotent_protection_enabled(False)

    fresh = SettingsService()
    assert fresh.get_kanban_multi_review_enabled() is False
    assert fresh.get_kanban_block_human_input_enabled() is False
    assert fresh.get_kanban_idempotent_protection_enabled() is False
...
def test_kanban_settings_api_get_put(monkeypatch, tmp_path):
    ...
    assert response.get_json()["settings"] == _default_settings()

    response = client.put("/api/kanban/settings", json={"auto_dispatch_enabled": True})
    assert response.status_code == 200
    assert response.get_json()["settings"] == _default_settings(auto_dispatch_enabled=True)

    response = client.get("/api/kanban/settings")
    assert response.get_json()["settings"]["auto_dispatch_enabled"] is True


def test_kanban_settings_api_partial_collaboration_update(monkeypatch, tmp_path):
    session_local = _session_local(tmp_path)
    monkeypatch.setattr("app.db.repositories.SessionLocal", session_local)
    monkeypatch.setattr(kanban_controller, "settings_service", SettingsService())
    app = Flask(__name__)
    ...
    response = client.put(
        "/api/kanban/settings",
        json={"multi_review_enabled": False, "idempotent_protection_enabled": False},
    )
    assert response.status_code == 200
    settings = response.get_json()["settings"]
    assert settings["multi_review_enabled"] is False
    assert settings["block_human_input_enabled"] is True
    assert settings["idempotent_protection_enabled"] is False
