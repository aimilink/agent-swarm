...
(async () => {
 ...
 try {
  ...
  await page.route('http://chat.test/**', async route=>{
   ...
   if(url.pathname==='/') return route.fulfill({contentType:'text/html',body:`<section id="view-chat"><select id="chat-agent"></select><button id="chat-new">新建</button><div id="chat-history"></div><h2 id="chat-title"></h2><div id="chat-messages"></div><p id="chat-status"></p><form id="chat-form"><textarea id="chat-input"></textarea><button id="chat-send">发送</button></form></section>`});
  ...
  await page.waitForFunction(()=>document.querySelectorAll('.chat-message').length===2);
  await page.selectOption('#chat-agent','b');
