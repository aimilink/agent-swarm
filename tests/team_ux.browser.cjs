...
(async()=>{
  ...
  try {
    ...
    assert.equal(await page.locator('#teams-manage-modal').getAttribute('data-mode'),'edit');
    assert.equal(await page.locator('#teams-manage-list').isVisible(),false,'team edit dialog does not nest the team list');
    ...
    await page.waitForFunction(()=>!document.querySelector('#create-team-form button[type="submit"]').disabled);
    assert.equal(await page.locator('#teams-manage-modal').getAttribute('data-mode'),'create');
    assert.equal(await page.locator('#teams-manage-list').isVisible(),false,'team create dialog stays focused on creation');
    ...
    assert.equal(await page.locator('#swarm-page-title').innerText(),'任务看板');
    const laneHeaders=await page.locator('#board-kanban-columns .board-column__header').evaluateAll(items=>items.map(item=>getComputedStyle(item).backgroundColor));
    assert.equal(new Set(laneHeaders).size,4,'each task status swimlane has a distinct header');
    assert.equal(await page.locator('#kanban-task-input').evaluate(el=>el.tagName),'TEXTAREA');
    assert.ok(await page.locator('#kanban-task-input').evaluate(el=>el.clientHeight)>=60,'task composer is taller');
    assert.ok(await page.locator('#kanban-task-input').evaluate(el=>parseFloat(getComputedStyle(el).paddingLeft))>=44,'task hint clears the icon');
