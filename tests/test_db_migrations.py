...
def test_ensure_runtime_schema_adds_long_running_task_columns(tmp_path):
    ...
    with engine.connect() as connection:
        ...
        agent_columns = {row[1] for row in connection.execute(text("PRAGMA table_info(agents)"))}
        message_columns = {row[1] for row in connection.execute(text("PRAGMA table_info(messages)"))}
    ...
    assert {"team_id"} <= agent_columns
    assert {"team_id"} <= message_columns
