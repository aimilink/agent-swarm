...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('#project-files .project-tree-directory summary').filter({hasText:'docs'}).count(),1);
  ...
  console.log('PASS: project/team many-to-many UI, agent filtering, iterations, IDE-style file tree, right-side Agent terminal, live project/task workspaces, online artifact preview, project isolation, mobile layout');
