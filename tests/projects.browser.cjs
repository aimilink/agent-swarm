...
(async()=>{
 ...
 try {
   ...
   if(url.pathname.startsWith('/api/projects/')) {
    ...
    if(parts[4]==='workspace'){
      ...
      const allFiles=[
        ...
        {path:'docs/report.csv',name:'report.csv',size:120,modified_ns:1,preview_type:'text',mime_type:'text/csv',is_artifact:false,artifact:null},
    ...
    if(parts[4]==='preview'){
      ...
      if(requested==='docs/report.csv')return route.fulfill({json:{ok:true,path:requested,preview_type:'text',mime_type:'text/csv',size:120,modified_ns:1,encoding:'utf-8',content:'name,description,status\r\nAlpha,"包含,逗号",done\r\nBeta,"多行\n内容","say ""hi"""'}});
  ...
  await page.waitForFunction(()=>document.querySelectorAll('#project-files [data-workspace-open]').length===5);
  ...
  assert.equal(await page.locator('[data-workspace-open="docs/report.csv"] .workspace-tree__file-icon--table').innerText(),'CSV');
  ...
  assert.equal(await page.locator('#project-file-count').innerText(),'5');
  ...
  assert.equal(await page.locator('#project-file-count').innerText(),'1/5');
  ...
  await page.locator('[data-workspace-open="docs/report.csv"]').click();
  await page.locator('#project-preview-content .project-csv-table').waitFor();
  assert.deepEqual(await page.locator('#project-preview-content .project-csv-table thead th').allInnerTexts(),['#','name','description','status']);
  assert.equal(await page.locator('#project-preview-content .project-csv-table tbody tr').count(),2);
  assert.equal(await page.locator('#project-preview-content .project-csv-table tbody tr').nth(0).locator('td').nth(1).innerText(),'包含,逗号');
  assert.equal(await page.locator('#project-preview-content .project-csv-table tbody tr').nth(1).locator('td').nth(1).innerText(),'多行\n内容');
  assert.equal(await page.locator('#project-preview-content .project-csv-table tbody tr').nth(1).locator('td').nth(2).innerText(),'say "hi"');
  assert.match(await page.locator('#project-preview-content .project-csv-summary').innerText(),/2 行数据 · 3 列/);
  ...
  assert.match(await page.locator('#project-preview-content pre').innerText(),/包含,逗号/);
