...
(async()=>{
 ...
 try {
  ...
  const createProjectDetails=page.locator('.project-create-details');
  if(await createProjectDetails.getAttribute('open')===null) await createProjectDetails.locator('summary').click();
  ...
  assert.equal(await page.locator('#project-tasks').isVisible(),true);
  await page.reload();
