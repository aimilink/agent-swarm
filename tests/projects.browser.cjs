...
(async()=>{
 ...
 try {
  ...
  await page.route('**/*',async route=>{
   ...
   if(url.pathname.startsWith('/api/projects/')) {
    ...
    if(parts[4]==='tasks') {
     ...
     const iteration=tasks.filter(t=>t.pid===pid).length+1; tasks.push({kanban_task_id:'kb_1',pid,local_type:'user_task',kanban_role:'parent',kanban_status:'ready',assignee_profile:'lead',metadata:{task_title:req.postDataJSON().content,project_iteration:iteration}});
    ...
    const projectTasks=tasks.filter(t=>t.pid===pid); return route.fulfill({json:{ok:true,project:projects.find(p=>p.project_id===pid),tasks:projectTasks,artifacts:artifacts.filter(a=>a.pid===pid),current_iteration:projectTasks.length,next_iteration:projectTasks.length+1}});
   }
  ...
  assert.ok((await page.locator('[data-project-task]').innerText()).includes('迭代 1'));
  assert.ok((await page.locator('#project-workspace').innerText()).includes('/workspace/projects/demo'));
  ...
  await page.waitForFunction(()=>document.querySelectorAll('#project-artifacts article').length===1);
  assert.ok(await page.locator('#overview-projects').innerText().then(t=>t.includes('/workspace/projects/demo')));
  ...
  console.log('PASS: project visibility, automatic selection, iteration status, task retry, artifact registration, project isolation, mobile layout');
