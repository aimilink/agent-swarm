...
- 团队 Token 用量正确读取 API 的 `usage` 结果；即使当前周期没有日志，也按团队显示 0 Token、0 次调用，不再误报“暂无用量数据”。
- 项目与任务工作空间改为 IDE 风格文件树、内容预览和右侧工具栏；右侧终端入口按当前任务或项目参与团队打开对应 Agent 的 Hermes 终端。
- 恢复 UI 覆盖样式加载；团队弹窗、小屏任务栏适配，任务卡支持 Enter/空格打开。
- 工作台增加统一系统健康状态；手动重检失败会保留上次结果并标记数据过期，异常组件可跳转到对应处理页面。
...
主入口沿用现有 Flask 模板与业务接口，接入 232px 侧栏、64px 页头、SVG 导航、白色卡片和蓝色主操作样式。生产样式和公共交互位于 `app/static/ui/agentswarm.css`、`agentswarm.js`。
