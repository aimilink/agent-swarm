...
2. 点击“新建项目”，填写名称以及目标、预期交付和验收标准，并多选参与团队。一个项目可关联多个团队，同一团队也可出现在多个项目中。
...
5. 在项目页“新建迭代任务”选择 Agent 并填写本轮目标、已有资料和预期交付路径；每次提交自动递增迭代编号，需要组织协作时选择 Leader。该入口沿用现有团队就绪、运行状态及 Worker 直派约束。
...
8. 进入页面时自动打开上次选择的项目；首次进入自动打开最近创建的项目。首页项目概览和项目列表显示参与团队，项目侧栏“团队参与项目”按团队反向汇总项目；详情页“管理参与团队”可随时调整关联。关联团队后，迭代任务只能选择参与团队中的 Agent，跨团队委派也只能发给参与团队。项目页“交付产物”集中展示记录。工作空间浏览器可在“项目工作空间”和“任务工作空间”之间切换：项目范围展示全部资料，任务范围只展示登记到该任务的产物。文本、代码、图片和 PDF 可在线预览，其他格式可下载。页面打开期间每 3 秒同步文件变化，当前预览文件被修改后自动更新。工作空间采用可筛选的文件树、内容区和右侧工具栏布局；目录的展开状态与文件树滚动位置在实时同步时保持不变，支持全部展开、全部折叠和 `/` 快捷聚焦筛选；右侧终端按钮会在当前任务下选择任务执行 Agent，在项目范围选择参与团队中运行中的 Agent，并打开该 Agent 的 Hermes 终端。登记表示文件已提交，Leader 仍需检查文件和验证结果，不能把登记当成验收通过。点击“刷新项目”会同时刷新项目信息和工作空间。
...
| 方法 | 路径 | 用途 |
|---|---|---|
| GET / POST | `/api/projects` | 列出项目 / 创建项目（`name`, `description`, `team_ids[]`） |
| GET | `/api/projects/<project_id>` | 项目详情、参与团队、关联任务和登记产物 |
| PUT | `/api/projects/<project_id>/teams` | 替换参与团队（`team_ids[]`） |
| POST | `/api/projects/<project_id>/tasks` | 创建项目任务（`content`, `to_agent_id`） |
| GET | `/api/projects/<project_id>/files` | 兼容接口：列出项目文件路径 |
| GET | `/api/projects/<project_id>/workspace?task_id=` | 获取项目或指定任务的工作空间文件、产物及预览类型 |
| GET | `/api/projects/<project_id>/preview?path=docs/design.md` | 在线预览项目内的文本、代码、图片或 PDF |
...
`projects` 表保存项目名称、目标、目录和创建时间；`project_teams` 保存项目与团队的多对多关联；`project_artifacts` 保存登记记录。任务的 `kanban_task_links.metadata` 保存 `project_id` 和 `project_workspace`，沿用现有持久化机制。新关联表会在服务启动时自动创建，存量项目保持未关联状态。
