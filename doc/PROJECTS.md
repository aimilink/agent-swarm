...
8. 进入页面时自动打开上次选择的项目；首次进入自动打开最近创建的项目。首页项目概览和项目列表显示参与团队，项目侧栏“团队参与项目”按团队反向汇总项目；详情页“管理参与团队”可随时调整关联。关联团队后，迭代任务只能选择参与团队中的 Agent，跨团队委派也只能发给参与团队。项目页“交付产物”集中展示记录。登记表示文件已提交，Leader 仍需检查文件和验证结果，不能把登记当成验收通过。点击“刷新项目”获取最新任务、文件和产物。
