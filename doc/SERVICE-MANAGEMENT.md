...
项目根目录的 `agentweave` 命令用于管理 Web 服务：
...
未安装全局命令时，可以在项目根目录执行 `./agentweave <command>`。
...
没有检测到用户级 `agentweave.service` 时，命令直接管理项目进程：
...
| 路径 | 用途 |
|---|---|
| `.run/agentweave.pid` | 当前服务 PID。 |
| `.run/agentweave.log` | 标准输出与错误日志。 |
...
```bash
tail -f .run/agentweave.log
```

## 4. systemd 模式

如果 `systemctl --user cat agentweave.service` 成功，管理命令会自动转交给用户级
systemd：

```text
agentweave start    -> systemctl --user start agentweave.service
agentweave stop     -> systemctl --user stop agentweave.service
agentweave status   -> systemctl --user --no-pager status agentweave.service
agentweave restart  -> systemctl --user restart agentweave.service
```
...
| 变量 | 默认值 | 用途 |
|---|---|---|
| `AGENTWEAVE_HOME` | 命令文件所在项目目录 | 显式指定项目根目录。 |
| `AGENTWEAVE_RUN_DIR` | `<项目>/.run` | 修改 PID 和锁目录。 |
| `AGENTWEAVE_LOG_FILE` | `<运行目录>/agentweave.log` | 修改普通进程日志路径。 |
...
| 启动后立即失败 | 查看 `.run/agentweave.log`，检查端口、依赖和 `.env`。 |
| `status` 显示停止，但页面仍可访问 | 可能存在 systemd 之外手工启动的旧进程；核对监听端口和启动用户。 |
| 提示另一个管理命令正在运行 | 等待当前 start、stop 或 restart 完成后重试。 |
| systemd 模式状态异常 | 执行 `systemctl --user status agentweave` 和 `journalctl --user -u agentweave -n 100`。 |
