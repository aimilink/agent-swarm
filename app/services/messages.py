...
def _kanban_service_for_leader(runtime_store: RuntimeStore, agent: dict):
    """按 agent 所属团队返回 (KanbanService, board_name)。未分组走默认板。"""
    from .kanban import KANBAN_BOARD, kanban_service, kanban_service_for_board

    team_id = (agent or {}).get("team_id")
    if team_id:
        team = runtime_store.find_team(team_id)
        board_name = (team or {}).get("board_name") or ""
        if board_name and board_name != KANBAN_BOARD:
            return kanban_service_for_board(board_name), board_name
    return kanban_service, KANBAN_BOARD
