...
from ..services.kanban import KanbanError, kanban_service, kanban_service_for_board
...
def _service_for_task(task_id: str):
    """多团队：按 link.metadata.board 选 KanbanService（无标记回退默认板）。"""
    link = store.find_kanban_task_link(kanban_task_id=task_id)
    board = ((link or {}).get("metadata") or {}).get("board") or ""
    return kanban_service_for_board(board) if board else kanban_service
...
def task_runs(task_id: str):
    try:
        return jsonify({"ok": True, "runs": _service_for_task(task_id).runs(task_id)})
...
def task_log(task_id: str):
    ...
    try:
        return jsonify({"ok": True, "log": _service_for_task(task_id).log(task_id, tail=tail)})
    except KanbanError as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500
...
def unblock_task(task_id: str):
    try:
        output = _service_for_task(task_id).unblock_task(task_id)
        link = store.find_kanban_task_link(kanban_task_id=task_id)
        if link:
...
def archive_task(task_id: str):
    ...
    try:
        cli_started = time.perf_counter()
        output = _service_for_task(task_id).archive_task(task_id)
        cli_elapsed = time.perf_counter() - cli_started
        store.update_kanban_task_link(task_id, kanban_status="archived")
        sync_worker.sync_once_async()
        logger.warning(
...
_KANBAN_BOOL_SETTINGS = {
    "auto_dispatch_enabled": settings_service.set_kanban_auto_dispatch_enabled,
    "multi_review_enabled": settings_service.set_kanban_multi_review_enabled,
    "block_human_input_enabled": settings_service.set_kanban_block_human_input_enabled,
    "idempotent_protection_enabled": settings_service.set_kanban_idempotent_protection_enabled,
}


@bp.get("/settings")
def get_settings():
    return jsonify({"ok": True, "settings": settings_service.kanban_settings_snapshot()})


@bp.put("/settings")
def update_settings():
    payload = request.get_json(silent=True) or {}
    provided = [key for key in _KANBAN_BOOL_SETTINGS if key in payload]
    if not provided:
        return jsonify({"ok": False, "error": "at least one settings field is required"}), 400
    for key in provided:
        if not isinstance(payload[key], bool):
            return jsonify({"ok": False, "error": f"{key} must be boolean"}), 400
        _KANBAN_BOOL_SETTINGS[key](payload[key])
    return jsonify({"ok": True, "settings": settings_service.kanban_settings_snapshot()})
