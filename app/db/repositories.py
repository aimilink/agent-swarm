...
class SQLitePersistence:
    ...
    def upsert_agent(self, agent: dict) -> None:
        with SessionLocal.begin() as session:
            ...
            record.team_id = agent.get("team_id") or None
    ...
    def upsert_team(self, team: dict) -> None:
        with SessionLocal.begin() as session:
            record = session.scalar(
                select(TeamRecord).where(TeamRecord.team_id == team["team_id"])
            )
            if record is None:
                record = TeamRecord(team_id=team["team_id"], slug=team["slug"], name=team["name"])
                session.add(record)
            record.slug = team.get("slug") or record.slug
            record.name = team.get("name") or record.name
            record.description = team.get("description") or ""
            record.board_name = team.get("board_name") or ""
            record.settings_json = _json_dumps(team.get("settings") or {})
            record.created_at = team.get("created_at")
            record.updated_at = team.get("updated_at")
            record.deleted_at = team.get("deleted_at")

    def soft_delete_team(self, team_id: str, deleted_at: str) -> None:
        with SessionLocal.begin() as session:
            record = session.scalar(select(TeamRecord).where(TeamRecord.team_id == team_id))
            if record is not None:
                record.deleted_at = deleted_at
    ...
    def load_teams(self) -> list[dict]:
        with SessionLocal() as session:
            records = session.scalars(
                select(TeamRecord)
                .where(TeamRecord.deleted_at.is_(None))
            ...
            return [
                {
                    ...
                    "name": record.name,
                    "description": record.description or "",
                    "board_name": record.board_name or "",
                    "settings": _json_loads(record.settings_json, {}),
                    "created_at": record.created_at or "",
    ...
    def _team_to_dict(self, record: TeamRecord) -> dict:  # pragma: no cover - 兼容预留
        return {
            "team_id": record.team_id,
            "slug": record.slug,
            "name": record.name,
